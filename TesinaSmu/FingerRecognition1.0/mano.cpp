...
//    int ypos=src.rows/1.6;
   ...
   // int lineChange=14;
 /*   string info= "Info figura:";
    putText(src,info,Point(ypos,xpos),fontFace,fontSize,fColor);
    xpos+=lineChange;
    info=string("Numero di difetti: ") + string(intToString(NrDiDifetti)) ;
    putText(src,info,Point(ypos,xpos),fontFace,fontSize  ,fColor);
    xpos+=lineChange;
    info=string("Altezza box, larghezza box ") + string(intToString(bRect_height)) + string(" , ") +  string(intToString(bRect_width)) ;
    putText(src,info,Point(ypos,xpos),fontFace,fontSize ,fColor);
    xpos+=lineChange;*/
    //info=string("E' una mano? ") + string(bool2string(isHand));
...
void Mano::AggiungiNumerotoImg(Immagine *m){
    ...
    for(int i=0;i<numbers2Display.size();i++){
       // rectangle(m->src,Point(xPos,yPos),Point(xPos+offset,yPos+offset),numeroColore, 2);
...
void Mano::EliminaDifetti(Immagine *m){
    ...
    while( d!=difetti[cIdx].end() ) {
        ...
        if(distanzaP2P(ptStart, ptFar) > tolerance && distanzaP2P(ptEnd, ptFar) > tolerance && getAngolo(ptStart, ptFar, ptEnd  ) < angleTol ){
...
void Mano::RimuoviPuntiFinaliRidondanti(vector<Vec4i> newdifetti,Immagine *m){
    ...
    for(int i=0;i<newdifetti.size();i++){
        for(int j=i;j<newdifetti.size();j++){
            ...
            if(distanzaP2P(ptStart,ptEnd2) < tolerance ){
                Contorni[cIdx][startidx]=ptEnd2;
                break;
            }if(distanzaP2P(ptEnd,ptStart2) < tolerance ){
...
// metodo che controlla quando non ci sono
//difetti convessità
