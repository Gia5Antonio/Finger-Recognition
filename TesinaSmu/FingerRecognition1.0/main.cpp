...
void AttendiCoperturaPalmo(Immagine* m){
    ...
    for(int i =0;i<50;i++){
        ...
        string imgText=string("Copri i rettangoli con il palmo");
...
void FaiMedia(Immagine *m){
    ...
    for(int i=0;i<30;i++){
        m->cap >> m->src;
        flip(m->src,m->src,1);
        cvtColor(m->src,m->src,ORIGCOL2COL); //Converte immagine nello spazio dei colori RGB

        for(int j=0;j<NUMCAMPIONI;j++){
            getAvgColor(m,roi[j],avgColor[j]); //Per ogni campione (rettangolino) ottiene un valore medio di tinta, sfumatura e tono
            roi[j].DisegnaRettangolo(m->src);
        }
        cvtColor(m->src,m->src,COL2ORIGCOL); //Converte immagine nello spazion dei colori HSV
...
void initTrackbars(){
    for(int i=0;i<NUMCAMPIONI;i++){
        ...
        //12,7,30,40,80,80 default
    }
    createTrackbar("H-lower1","trackbars",&c_lower[0][0],255);
    createTrackbar("S-lower2)","trackbars",&c_lower[0][1],255);
    createTrackbar("V-lower3","trackbars",&c_lower[0][2],255);
    createTrackbar("H-upper1","trackbars",&c_upper[0][0],255);
    createTrackbar("S-upper2","trackbars",&c_upper[0][1],255);
    createTrackbar("V-upper3","trackbars",&c_upper[0][2],255);
}
...
void produciImgBianarie(Immagine *m){
    ...
    for(int i=0;i<NUMCAMPIONI;i++){
        ...
        inRange(m->srcLR,lowerBound,upperBound,m->bwList[i]); //Controlla se gli elementi di srcLR si trovano tra gli elementi di altri 2 array (lowerBound,upperBound)
    }
...
void CreaContorni(Immagine *m, Mano* manog){
    Mat aBw;
    pyrUp(m->bw,m->bw); //Campiona immagine
    m->bw.copyTo(aBw);
    ...
    if(manog->cIdx!=-1){
        ...
        approxPolyDP( Mat(manog->hullP[manog->cIdx]), manog->hullP[manog->cIdx], 18, true ); //Approssima una curva o un poligono con un'altra curva con meno vertici, in modo che la distanza tra di loro è <= alla precisione specificata(18), true: curva chiusa
        if(manog->Contorni[manog->cIdx].size()>3 ){
            ...
            manog->EliminaDifetti(m); //Un difetto è respinto se respinto se l'angolo tra le linee che vanno dal difetto ai vicini vertici poligono convesso ha lunghezza <0.40lbb ed è >80°
        }
        bool isHand=manog->RilevaSeMano();
          //Stampa informazioni sulla mano
        if(isHand){
              manog->getPuntaDita(m);
              manog->DisegnaPuntaDita(m);
...
int main(){
   ...
   // out.open("out.avi", CV_FOURCC('M', 'J', 'P', 'G'), 15, m.src.size(), true);
    ...
    for(;;){
        ...
        pyrDown(m.src,m.srcLR);//Leviga e campiona l' immagine, srcLR sarà l'immagine di output
        blur(m.srcLR,m.srcLR,Size(3,3)); //Elimina sfumature, srcLR sarà l'immagine di output
        cvtColor(m.srcLR,m.srcLR,ORIGCOL2COL); //Converte immagine nello spazio HSV
        produciImgBianarie(&m); //Produce le immagini binarie e leviga l'immagine utilizzando un filtro mediano
        cvtColor(m.srcLR,m.srcLR,COL2ORIGCOL); //Converte immagine nello spazio RGB
        CreaContorni(&m, &manog); //Crea contorni (sia rettangolo esterno che quello dell'ellissoide)
        manog.getNumeroDita(&m); //Stampa a video il numero di dita rilevate
        showWindows(m);
        //out << m.src;
       // imwrite("C:\\final_result.jpg",m.src);
